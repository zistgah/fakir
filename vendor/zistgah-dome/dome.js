...
/* ---------- 3D ---------- */
const R=10;
let camera,renderer,scene,G,shellTex,dialTex,shellMat,ok3d=false,D=null;
...
function initGL(){
  try{
    ...
    scene.add(new THREE.Points(gg2,new THREE.PointsMaterial({color:0xffe9b0,size:0.3,
    ...
    for(const w of ZP.worlds){
      ...
      if(w.habitat){                                          // a small dome, floating over it
        const hd=w.habitat.domeR, hy=w.r+hd*0.05;
    ...
    BND=boundsFor('dome',ZP);F=flightInit();
...
function rebuildStars(){
  ...
  for(let i=0;i<N;i++){let x,y,z;do{x=Math.random()*2-1;y=Math.random()*2-1;z=Math.random()*2-1}
    ...
    const L2=55/Math.hypot(x,y,z);sp[i*3]=x*L2;sp[i*3+1]=y*L2;sp[i*3+2]=z*L2}
  const sg=new THREE.BufferGeometry();
  sg.setAttribute('position',new THREE.BufferAttribute(sp,3));
  starsMesh=new THREE.Points(sg,new THREE.PointsMaterial({color:0xf2ecdd,size:0.14,
...
function boundsFor(zone,ZP){
  ...
  return {yMin:0.2,yMax:dR*0.98,kind:"world",world:w,
    cx:w.at[0], cz:w.at[2], cy:w.at[1]+w.r,        // the habitat floor sits on the surface
...
function dockPoint(w,ZP){
  ...
  return [w.at[0]-w.at[0]/n*(w.r*0.15), w.at[1]+w.r+w.habitat.domeR*0.35,
...
function flightStep(F,u,dt,B,EMB){
  ...
  F.yaw  += u.yaw*RATE*dt;
  F.pitch = F.pitch + (u.pitch*0.5 - F.pitch)*Math.min(1,LEVEL*dt);
  F.roll  = F.roll  + (u.roll *0.5 - F.roll )*Math.min(1,LEVEL*dt);
    ...
    if(PILOT&&F&&BND){
      ...
      if(TRANSIT){                                   /* the rocket flies itself */
        ...
        if(TRANSIT.u>=1){                            /* arrival: the world dictates the body */
          ...
          const w=ZP.worlds.find(x=>x.id===ZONE);
          F=flightInit();F.x=w.at[0];F.z=w.at[2];F.y=w.at[1]+w.r+0.6;
          TRANSIT=null;
...
const ART_SUB=Object.assign({},{"Kaivalya AGI":"Pursuing truth, well-being and wisdom.",
  ...
  "Zistgah":"The living habitat — integrating technology, nature and humanity."},CFG.exhibitSubs||{}); /*ZDOME-SEAM:exhibitSubs*/
...
function hitAt(cx2,cy2){
  ...
  for(const wm of worldMeshes){                      // Moon / Mars habitat domes
    ...
    const s2=wm.center.clone().project(camera.clone());
    if(s2.z>1)continue;
  ...
  for(const p of pedBase){
    ...
    if(s.z<1&&Math.hypot(cx2-sx,cy2-sy)<Math.max(38,innerWidth*0.045))
      return {kind:'ped',name:p.name,sub:ART_SUB[p.name],sx,sy};
  }
...
function showPop(hit){
  ...
  el.innerHTML=`<b>${hit.name}</b><span class="s2">${hit.sub}</span>
    <button>${hit.kind==='frac'?'◈ Open the fractal':
               hit.kind==='world'?(PILOT?'▲ Launch':'⟡ Seed AI'):'⟡ Seed AI'}</button>`;
  el.querySelector('button').onclick=()=>
    hit.kind==='frac'?openFractal():
    hit.kind==='world'?launchTo(hit.id):seed(hit.name,hit.sub);
...
const DOMAINS=CFG.domains/*ZDOME-SEAM:domains*/||[["Artificial Intelligence","Consciousness-aligned intelligence systems for collective good."],
...
document.getElementById('domains').innerHTML=DOMAINS.map(([b,p])=>
 `<div class="dom"><b>${b}</b><p>${p}</p></div>`).join('');
const MARK=CFG.marks/*ZDOME-SEAM:marks*/||{
...
const ART=CFG.art/*ZDOME-SEAM:art*/||[["Kaivalya AGI","kaivalya","Pursuing truth, well-being and wisdom."],
...
document.getElementById('exhibits').innerHTML=ART.map(([b,m,p],i)=>
 `<article class="ex"><svg viewBox="0 0 86 86" aria-hidden="true">${MARK[m]}</svg>
  <b>${b}</b><p>${p}</p><button class="seed" data-i="${i}">⟡ Seed AI</button></article>`).join('');
document.querySelectorAll('.seed').forEach(btn=>btn.onclick=()=>{
  const [b,,p]=ART[+btn.dataset.i];seed(b,p);});
const THREADS=CFG.threads/*ZDOME-SEAM:threads*/||[
...
document.getElementById('threads').innerHTML=THREADS.map(([b,p,links])=>
